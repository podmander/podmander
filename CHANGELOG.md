...
- Added TOML-native service port mappings while preserving the existing string
  form (#204)

### Fixed

- Hardened controller bind-address and database timestamp parsing invariants so
  overlong bind addresses and malformed ISO8601 timestamps fail before unsafe
  fixed-string slicing (#191)

## [0.2.5] - 2026-07-13

### Changed

- Tightened node ID, row ID, and port number domain types so negative IDs and
  out-of-range ports are no longer representable where the domain forbids them
  (#188)
...
- Config parser now returns clear parse errors for malformed TOML value shapes
  instead of leaking TOML accessor precondition exceptions (#190)

## [0.2.3] - 2026-07-11

### Changed

- Consolidated agent repository timestamp conversion onto the shared database
  time helpers (#186)
...
- Removed unnecessary mutating parameter modes from controller join-token
  generation and service row mapping helpers (#192)

## [0.2.1] - 2026-07-11

### Changed

- Made controller and agent message handlers store their owning instances as
  not-null references (#187)
