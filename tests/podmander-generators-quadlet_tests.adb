...
package body Podmander.Generators.Quadlet_Tests is
   ...
   procedure Test_Render_Minimal (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "[Container]") > 0,
         "Output should contain [Container] section");
      Assert
        (Ada.Strings.Fixed.Index (Output, "Image=nginx:latest") > 0,
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "Environment=FOO=bar") > 0,
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "Environment=FOO=bar") > 0,
         "Output should contain Environment=FOO=bar");
      Assert
        (Ada.Strings.Fixed.Index (Output, "Environment=BAZ=qux") > 0,
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "PublishPort=8080:80") > 0,
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "PublishPort=8080:80") > 0,
         "Output should contain PublishPort=8080:80");
      Assert
        (Ada.Strings.Fixed.Index (Output, "PublishPort=443:443") > 0,
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "Volume=/data:/data") > 0,
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "[Container]") > 0,
         "Output should contain [Container] section");
      Assert
        (Ada.Strings.Fixed.Index (Output, "Image=myapp:latest") > 0,
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "[Unit]") > 0,
   ...
   begin
      Assert
        (Ada.Strings.Fixed.Index (Output, "[Install]") > 0,
   ...
   begin
      ...
      Assert
        (Unit_Pos < Container_Pos,
         "[Unit] section should appear before [Container]");
      Assert
        (Container_Pos < Install_Pos,
   ...
   begin
      ...
      Assert
        (Ada.Directories.Exists (FPath), "File " & FPath & " should exist");
   ...
   begin
      ...
      Assert
        (Ada.Directories.Exists (Dir), "Directory " & Dir & " should exist");
      Assert
        (Ada.Directories.Exists (FPath), "File " & FPath & " should exist");
   ...
   begin
      ...
      Assert
        (Ada.Strings.Unbounded.To_String (Actual) = Expected,
   ...
   procedure Test_Golden_Minimal (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
      Config : constant Service_Definition :=
        (Service_Name  => Null_Unbounded_String,
           ...
           [1      =>
            ...
            2      =>
              (Key   => To_Unbounded_String ("BAZ"),
               Value => To_Unbounded_String ("qux")),
            3      =>
              (Key   => To_Unbounded_String ("APP_ENV"),
