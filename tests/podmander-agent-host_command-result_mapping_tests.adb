...
package body Podmander.Agent.Host_Command.Result_Mapping_Tests is
   ...
   overriding function Name
     (T : Mapping_Test) return AUnit.Message_String
   is (AUnit.Format ("Host_Command.Result_Mapping"));

   overriding procedure Register_Tests (T : in out Mapping_Test);
   ...
   is
      ...
      Result : constant Command_Result :=
        (State        => Exited,
         ...
         Error_Output => Empty);
   begin
      Assert (RM.To_Result_Code (Result) = RC.Ok,
              "Exited(0) should map to Ok");
   ...
   is
      ...
      Result : constant Command_Result :=
        (State        => Exited,
         ...
         Error_Output => Empty);
   begin
      Assert (RM.To_Result_Code (Result) = RC.Failed,
   ...
   is
      ...
      Result : constant Command_Result :=
        (State        => Error,
         ...
         Error_Output => Empty);
   begin
      Assert (RM.To_Result_Code (Result) = RC.Unavailable,
   ...
   is
      ...
      Result : constant Command_Result :=
        (State        => Crashed,
         ...
         Error_Output => Empty);
   begin
      Assert (RM.To_Result_Code (Result) = RC.Internal,
   ...
   overriding procedure Register_Tests (T : in out Mapping_Test) is
      use AUnit.Test_Cases.Registration;
   begin
      Register_Routine
        (T, Test_Exited_Zero_Maps_Ok'Access,
         "Exited(0) maps to Ok");
      Register_Routine
        (T, Test_Exited_Nonzero_Maps_Failed'Access,
         "Exited(non-zero) maps to Failed");
      Register_Routine
        (T, Test_Error_Maps_Unavailable'Access,
         "Error maps to Unavailable");
      Register_Routine
        (T, Test_Crashed_Maps_Internal'Access,
         "Crashed maps to Internal");
      Register_Routine
        (T, Test_Terminated_Maps_Internal'Access,
