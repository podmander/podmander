...
package body Podmander.Controller.Actual_State.Repository_Tests is
   ...
   begin
      ...
      Assert (Natural (All_E.Length) = 1,
              "Should have one entry after upsert");
   end Test_Upsert_Inserts;

   procedure Test_Upsert_Updates
     (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
      D    : DB.DB_Handle := DB.Open (":memory:");
   ...
   begin
      ...
      Assert (All_E.First_Element.Version = 2,
   ...
   begin
      ...
      Repo.Upsert (D, E3);
      All_E := Repo.Get_All (D);
      Assert (Natural (All_E.Length) = 3,
              "Should have 3 entries after 3 upserts");
   end Test_Get_All_Returns_All;

   procedure Test_Get_For_Service
     (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
      D      : DB.DB_Handle := DB.Open (":memory:");
   ...
   begin
      ...
      Assert (Natural (Svc_Entries.Length) = 2,
   ...
   begin
      ...
      Assert (Natural (Svc_Entries.Length) = 0,
   ...
   begin
      ...
      All_E := Repo.Get_All (D);
      Assert (Natural (All_E.Length) = 1,
              "Should have 1 entry after remove");
      Assert (To_String (All_E.First_Element.Service_Name) = "db",
   ...
   begin
      Register_Routine
        (T, Test_Upsert_Inserts'Access,
         "Upsert inserts a new actual_state entry");
      Register_Routine
        (T, Test_Upsert_Updates'Access,
         "Upsert updates an existing actual_state entry");
      Register_Routine
        (T, Test_Get_All_Returns_All'Access,
         "Get_All returns all actual_state entries");
      Register_Routine
        (T, Test_Get_For_Service'Access,
         "Get_For_Service returns entries for a specific service");
      Register_Routine
        (T, Test_Get_For_Service_Unknown'Access,
         "Get_For_Service returns empty vector for unknown service");
      Register_Routine
        (T, Test_Remove'Access,
         "Remove deletes an actual_state entry");
      Register_Routine
        (T, Test_Remove_Unknown_Noop'Access,
