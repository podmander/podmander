...
package body Podmander.Controller.Service.Repository_Tests is
   ...
   procedure Test_Create_Version (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D      : DB.DB_Handle := DB.Open (":memory:");
      Svc    : constant Podmander.Controller.Service.Service := Repo.Create (D, My_Service);
      SV     : Podmander.Controller.Service_Version := Make_Version (1, Svc.Id);
      Loaded : Podmander.Controller.Service_Version;
   begin
      Repo.Create_Version (D, SV);
      Loaded := Repo.Get_Version (D, Svc.Id, Podmander.Controller.Service_Version_Type(1));
      ...
      Assert (Loaded.Version = Podmander.Controller.Service_Version_Type(1), "Version should be 1");
      ...
      Assert (To_String (Loaded.Env (1).Value) = "bar", "Env value should match");
      ...
      Assert (To_String (Loaded.Volumes (1).Host) = "/host/data", "Volume host should match");
      Assert (To_String (Loaded.Volumes (1).Container) = "/container/data", "Volume container should match");
      Assert (Format_Time (Loaded.Created_At) = Format_Time (SV.Created_At), "Created_At should match");
   end Test_Create_Version;

   procedure Test_Create_Version_Duplicate_Raises (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D         : DB.DB_Handle := DB.Open (":memory:");
      Svc       : constant Podmander.Controller.Service.Service := Repo.Create (D, My_Service);
      SV        : Podmander.Controller.Service_Version := Make_Version (1, Svc.Id);
   ...
   begin
      ...
      Assert (Got_Error, "Duplicate version should have raised Database_Error");
   end Test_Create_Version_Duplicate_Raises;

   procedure Test_Get_Version_Not_Found (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D         : DB.DB_Handle := DB.Open (":memory:");
      Got_Error : Boolean := False;
   begin
      begin
         declare
            Ignored : Podmander.Controller.Service_Version := Repo.Get_Version (D, Podmander.Controller.Service_Id_Type(999), Podmander.Controller.Service_Version_Type(1));
         begin
            null;
      ...
      exception
         when E : DB.Database_Error =>
            declare
               Err : constant DB.Error_Info := DB.Parse_Error (E);
            begin
               Assert (Err.Kind = DB.Not_Found, "Get_Version on unknown should raise Not_Found");
               Got_Error := True;
            end;
      end;
      Assert (Got_Error, "Get_Version on unknown should have raised Database_Error");
   end Test_Get_Version_Not_Found;

   procedure Test_Get_Latest_Version (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D      : DB.DB_Handle := DB.Open (":memory:");
      Svc    : constant Podmander.Controller.Service.Service := Repo.Create (D, My_Service);
      SV1    : Podmander.Controller.Service_Version := Make_Version (1, Svc.Id);
      SV2    : Podmander.Controller.Service_Version := Make_Version (2, Svc.Id);
      SV3    : Podmander.Controller.Service_Version := Make_Version (3, Svc.Id);
   ...
   begin
      ...
      Assert (Latest.Version = Podmander.Controller.Service_Version_Type(3), "Latest version should be 3");
      Assert (To_String (Latest.Image) = "nginx:1.27", "Latest image should match version 3");
   end Test_Get_Latest_Version;

   procedure Test_Get_Latest_Version_Not_Found (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D         : DB.DB_Handle := DB.Open (":memory:");
      Got_Error : Boolean := False;
   begin
      ...
      exception
         when E : DB.Database_Error =>
            declare
               Err : constant DB.Error_Info := DB.Parse_Error (E);
            begin
               Assert (Err.Kind = DB.Not_Found, "Get_Latest_Version on unknown should raise Not_Found");
               Got_Error := True;
            end;
      end;
      Assert (Got_Error, "Get_Latest_Version on unknown should have raised Database_Error");
   end Test_Get_Latest_Version_Not_Found;

   procedure Test_Create_Version_With_Empty_Arrays (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D      : DB.DB_Handle := DB.Open (":memory:");
      Svc    : constant Podmander.Controller.Service.Service := Repo.Create (D, My_Service);
      SV     : Podmander.Controller.Service_Version := Make_Version (1, Svc.Id);
   ...
   procedure Test_Service_Create (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D       : DB.DB_Handle := DB.Open (":memory:");
   ...
   begin
      ...
      Assert (To_String (Created.Name) = My_Service_Name, "Service name should match");
   end Test_Service_Create;

   procedure Test_Service_Create_Idempotent (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D      : DB.DB_Handle := DB.Open (":memory:");
   ...
   begin
      ...
      Assert (To_String (Second.Name) = My_Service_Name, "Service name should match on second create");
   end Test_Service_Create_Idempotent;

   procedure Test_Service_Get_By_Name (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D       : DB.DB_Handle := DB.Open (":memory:");
      Created : Podmander.Controller.Service.Service;
   ...
   begin
      ...
      Assert (To_String (Found.Name) = My_Service_Name, "Get_By_Name should return same name");
   end Test_Service_Get_By_Name;

   procedure Test_Service_Get_By_Name_Not_Found (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D         : DB.DB_Handle := DB.Open (":memory:");
      Got_Error : Boolean := False;
   begin
      begin
         declare
            Ignored : Podmander.Controller.Service.Service := Repo.Get_By_Name (D, "nonexistent");
         begin
            null;
            pragma Unreferenced (Ignored);
         end;
      exception
         when E : DB.Database_Error =>
            declare
               Err : constant DB.Error_Info := DB.Parse_Error (E);
            begin
               Assert (Err.Kind = DB.Not_Found, "Get_By_Name on unknown should raise Not_Found");
               Got_Error := True;
            end;
      end;
      Assert (Got_Error, "Get_By_Name on unknown should have raised Database_Error");
   end Test_Service_Get_By_Name_Not_Found;

   procedure Test_Service_Get_By_Id (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D       : DB.DB_Handle := DB.Open (":memory:");
      Created : Podmander.Controller.Service.Service;
      Found   : Podmander.Controller.Service.Service;
   begin
      ...
      Assert (To_String (Found.Name) = My_Service_Name, "Get_By_Id should return same name");
   end Test_Service_Get_By_Id;

   procedure Test_Service_Get_By_Id_Not_Found (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      D         : DB.DB_Handle := DB.Open (":memory:");
      Got_Error : Boolean := False;
   begin
   ...
   begin
      Register_Routine
        (T, Test_Create_Version'Access, "Create a service version and verify it persists with all fields");
      Register_Routine
        (T, Test_Create_Version_Duplicate_Raises'Access, "Create duplicate version raises Constraint_Violation");
      Register_Routine (T, Test_Get_Version_Not_Found'Access, "Get_Version on nonexistent version raises Not_Found");
      Register_Routine (T, Test_Get_Latest_Version'Access, "Get_Latest_Version returns the highest version");
      Register_Routine
        (T, Test_Get_Latest_Version_Not_Found'Access, "Get_Latest_Version on unknown service raises Not_Found");
      Register_Routine
        (T, Test_Create_Version_With_Empty_Arrays'Access, "Create version with empty env/ports/volumes arrays");
      Register_Routine (T, Test_Service_Create'Access, "Create a new service and verify id and name");
      Register_Routine (T, Test_Service_Create_Idempotent'Access, "Create same service twice returns same id");
      Register_Routine (T, Test_Service_Get_By_Name'Access, "Get service by name returns correct fields");
      Register_Routine (T, Test_Service_Get_By_Name_Not_Found'Access, "Get service by unknown name raises Not_Found");
      Register_Routine (T, Test_Service_Get_By_Id'Access, "Get service by id returns correct fields");
      Register_Routine (T, Test_Service_Get_By_Id_Not_Found'Access, "Get service by unknown id raises Not_Found");
   end Register_Tests;
