...
package body Podmander.Agent.Host_Command_Tests is
   ...
   begin
      ...
      Assert
        (Contains (Result.Output, "err"),
         "Expected merged output to contain 'err'");
   end Test_Run_Stderr_Merged;

   procedure Test_Run_Shell_Echo (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
   ...
   procedure Test_Run_Many_Args (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
   ...
   begin
      Register_Routine
        (T,
         Test_Run_Echo_Success'Access,
         "Run echo returns Exited with output");
      Register_Routine
        (T,
         Test_Run_False_Nonzero'Access,
      ...
      Register_Routine
        (T,
         Test_Run_Stderr_Merged'Access,
         "Stderr merged into Output when Err_To_Out=True");
      Register_Routine
        (T, Test_Run_Shell_Echo'Access, "Run_Command_Shell echoes correctly");
      Register_Routine
        (T,
         Test_Run_Shell_Pipeline'Access,
         "Run_Command_Shell supports pipes");
      Register_Routine
        (T,
         Test_Run_Many_Args'Access,
