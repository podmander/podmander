...
package body Podmander.Controller_Tests is
   ...
   with record
      ...
      Last_Deploy_Cmd              :
        Podmander.Messages.Deployment_Commands.Deployment_Command;
      Last_Deploy_Res              :
        Podmander.Messages.Deployment_Results.Deployment_Result;
      Last_Status_Resp             :
        Podmander.Messages.Status_Responses.Status_Response;
      Last_Stack_Submission        :
        Podmander.Messages.Stack_Submissions.Stack_Submission;
      Last_Stack_Submission_Result :
        Podmander.Messages.Stack_Submission_Results.Stack_Submission_Result;
   end record;
   ...
   procedure Handle_Deployment_Command
     (H : in out Spy_Handler;
      M : Podmander.Messages.Deployment_Command_Type'Class) is
   begin
      H.Kind := Deployment_Command_Seen;
      H.Last_Deploy_Cmd :=
        Podmander.Messages.Deployment_Commands.Deployment_Command (M);
   end Handle_Deployment_Command;

   overriding
   procedure Handle_Deployment_Result
     (H : in out Spy_Handler;
      M : Podmander.Messages.Deployment_Result_Type'Class) is
   begin
      H.Kind := Deployment_Result_Seen;
      H.Last_Deploy_Res :=
        Podmander.Messages.Deployment_Results.Deployment_Result (M);
   ...
   procedure Handle_Stack_Submission
     (H : in out Spy_Handler;
      M : Podmander.Messages.Stack_Submission_Type'Class) is
   begin
      H.Kind := Stack_Submission_Seen;
      H.Last_Stack_Submission :=
   ...
   procedure Handle_Stack_Submission_Result
     (H : in out Spy_Handler;
      M : Podmander.Messages.Stack_Submission_Result_Type'Class) is
   begin
   ...
   is
      ...
      HB  : constant Heartbeat_Message :=
        (Connection_Id => To_Unbounded_String ("node-1"),
   ...
   begin
      ...
      Assert
        (To_String (Spy.Last_Heartbeat.Connection_Id) = "node-1",
         "Expected connection_id node-1");
   ...
   begin
      ...
      declare
         ...
         Node_Id : constant Node_Id_Type :=
           Node_Repo.Create_Or_Get (D, "persisted-agent");
      begin
   ...
   is
      ...
      Node_Id : constant Node_Id_Type :=
        Node_Repo.Create_Or_Get (Ctrl.DB, "web-1");
   ...
   is
      ...
      HB   : constant Podmander.Messages.Heartbeats.Heartbeat_Message :=
        (Connection_Id => To_Unbounded_String ("unknown"),
         Timestamp     => Ada.Calendar.Clock);
   begin
      H.Handle_Heartbeat (HB);
      declare
   ...
   is
      ...
      Res  :
        constant Podmander.Messages.Deployment_Results.Deployment_Result :=
          (Catalog_Id    => 0,
   ...
   begin
      ...
      Assert
        (True, "Handle_Deployment_Result should not crash without version");
   ...
   begin
      ...
      Assert
        (True,
         "Handle_Stack_Submission with valid secret should register service");
   exception
   ...
   is
      ...
      Cmd  : constant Podmander.Messages.Stack_Submissions.Stack_Submission :=
        (TOML              =>
           To_Unbounded_String
             ("[service.web]" & ASCII.LF & "ports = [""80:80""" & ASCII.LF),
   ...
   begin
      ...
      Assert
        (not Result.Ok, "Submit with invalid TOML should return Ok = False");
      Assert
