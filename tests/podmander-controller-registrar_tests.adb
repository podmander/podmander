...
package body Podmander.Controller.Registrar_Tests is
   ...
   function Make_Service_Definition
     (Name : String; Image : String; Desc : String; Wanted : String)
   ...
   begin
      ...
      Assert
        (Result.Error = Registrar.None, "Error should be None on success");

      -- Verify service row exists
      Found := Repo.Get_By_Name (D, Service_Name);
      Assert
        (To_String (Found.Name) = Service_Name, "Service name should match");
      Assert
