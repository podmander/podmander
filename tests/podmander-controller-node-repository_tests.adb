...
package body Podmander.Controller.Node.Repository_Tests is
   ...
   procedure Test_Create_Or_Get_New_Node
   ...
   begin
      Assert
        (Node_Id > 0,
         "Create_Or_Get should return a positive id for new node");
   end Test_Create_Or_Get_New_Node;

   --  Create_Or_Get returns existing id when called with same name
   procedure Test_Create_Or_Get_Idempotent
     (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
      D         : DB.DB_Handle := DB.Open (":memory:");
   ...
   begin
      Assert
        (Id_First = Id_Second,
         "Create_Or_Get should return same id for same name");
   end Test_Create_Or_Get_Idempotent;

   --  Create_Or_Get returns different ids for different names
   procedure Test_Create_Or_Get_Different_Names
     (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
      D        : DB.DB_Handle := DB.Open (":memory:");
   ...
   begin
      Assert
        (Id_Alpha /= Id_Beta,
         "Create_Or_Get should return different ids for different names");
   ...
   procedure Test_Load_By_Name_Existing
     (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
      D          : DB.DB_Handle := DB.Open (":memory:");
      Created_Id : constant Node_Id_Type :=
        Repo.Create_Or_Get (D, "worker-03");
      Loaded_Id  : constant Node_Id_Type := Repo.Load_By_Name (D, "worker-03");
   begin
      Assert
        (Loaded_Id = Created_Id,
         "Load_By_Name should return the same id as Create_Or_Get");
   end Test_Load_By_Name_Existing;

   --  Load_By_Name raises Not_Found for nonexistent node
   procedure Test_Load_By_Name_Not_Found
     (T : in out AUnit.Test_Cases.Test_Case'Class)
   is
      pragma Unreferenced (T);
      D         : DB.DB_Handle := DB.Open (":memory:");
            ...
            begin
               Assert
                 (Err.Kind = DB.Not_Found,
                  "Load_By_Name should raise Not_Found for missing node");
               Got_Error := True;
            end;
      end;
      Assert
        (Got_Error,
         "Load_By_Name should have raised Database_Error for missing node");
   ...
   begin
      Register_Routine
        (T,
         Test_Create_Or_Get_New_Node'Access,
         "Create_Or_Get creates a new node and returns its id");
      Register_Routine
        (T,
         Test_Create_Or_Get_Idempotent'Access,
         "Create_Or_Get returns same id for same name");
      Register_Routine
        (T,
         Test_Create_Or_Get_Different_Names'Access,
         "Create_Or_Get returns different ids for different names");
      Register_Routine
        (T,
         Test_Load_By_Name_Existing'Access,
         "Load_By_Name returns id of existing node");
      Register_Routine
        (T,
         Test_Load_By_Name_Not_Found'Access,
         "Load_By_Name raises Not_Found for nonexistent node");
   end Register_Tests;
