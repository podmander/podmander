...
- The CLI already queries cluster state for `podctl status`. Machine-readable output enables integration with external tools.
- The target audience may already have monitoring infrastructure. Podmander should integrate with it, not replace it.
- Automated remediation (webhook-triggered `podctl repair`) needs a non-interactive CLI mode.

## Decision

Podmander provides integration hooks for external monitoring systems rather than building its own. Specifically:

- **`podctl status --json`** — machine-readable cluster state for monitoring ingestion.
...
- No native Prometheus metrics endpoint — operators must wrap `podctl status --json` in an exporter script (native endpoint is a future extension).
...
- Why rejected: Not rejected — deferred. A native `podctl status --prometheus` endpoint is planned as a future extension. The JSON output is sufficient for the initial release.
