      ...
      if not Load_Result.Success then
         declare
            Error_Msg : constant String :=
              TOML.Format_Error (Load_Result);
         begin
            return (Success => False,
                    Message => To_Unbounded_String (Error_Msg));
         end;
      end if;
      ...
      begin
         ...
         if not Root.Has ("service") then
            return (Success => False,
                    Message =>
                      To_Unbounded_String
                        ("No [service] section found in config"));
         end if;
         ...
         begin
            ...
            begin
               ...
               Config.Image := To_Unbounded_String
                 (Service_Value.Get ("image").As_String);
                    ...
                    Message =>
                      To_Unbounded_String
                        ("Port container out of range (1-65535): "
                         & Trim (Config.Ports (I).Container'Image,
                             Ada.Strings.Both)));
         end if;
      end loop;
      ...
      for I in 1 .. Config.Volumes_Count loop
         if Config.Volumes (I).Host = Null_Unbounded_String then
            return (Success => False,
                    Message =>
                      To_Unbounded_String
                        ("Volume host path must not be empty"));
         end if;
