...
package body Podmander.Controller.Message_Handlers is
   ...
   procedure Send_Status_Query (H : in out Controller_Handler; Node_Id : String) is
   ...
   begin
      if not Podmander.Enrollment.Secret_Matches (H.Ctrl.Config.Enrollment, To_String (Req.Enrollment_Secret)) then
         Podmander.Logging.Warning ("controller", "Invalid enrollment secret from agent """ & Name & """");
         return;
      end if;
      ...
      begin
         ...
         Podmander.Logging.Info ("controller", "Registered agent """ & Name & """ as " & Node_Id);
      end;

      if H.Ctrl.Socket.Is_Valid then
         declare
            Reply     : constant Registration_Response := (Node_Id => To_Unbounded_String (Node_Id));
   ...
   overriding
   procedure Handle_Heartbeat (H : in out Controller_Handler; M : Podmander.Messages.Heartbeat_Message_Type'Class) is
   ...
   begin
         ...
         begin
            if To_String (Info.Node_Id) = Node_Id then
               ...
               if Info.State /= Podmander.Types.Registered then
                  ...
                  Agent.Repository.Set_State (H.Ctrl.DB, Info);
                  Podmander.Logging.Info ("controller", "Agent " & Node_Id & " reconnected");
               end if;
               Agent.Repository.Touch (H.Ctrl.DB, Info);
               Podmander.Logging.Debug ("controller", "Heartbeat from " & Node_Id);
               exit;
            end if;
         end;
      end loop;
      ...
      if not Found then
         Podmander.Logging.Warning
           ("controller", "Heartbeat from unregistered agent " & To_String (H.Identity) & ", ignoring");
      end if;
   end Handle_Heartbeat;

   overriding
   procedure Handle_Deploy_Command (H : in out Controller_Handler; M : Podmander.Messages.Deploy_Command_Type'Class) is
      pragma Unreferenced (H, M);
   begin
      Podmander.Logging.Warning ("controller", "Deploy_Command is controller-to-agent only");
   end Handle_Deploy_Command;

   overriding
   procedure Handle_Deploy_Result (H : in out Controller_Handler; M : Podmander.Messages.Deploy_Result_Type'Class) is
      ...
      if Result.Catalog_Id > 0 then
         ...
         if Result.Code = Podmander.Messages.Result_Codes.Ok then
            declare
               ...
               Cat_Entry : constant Podmander.Controller.Service_Catalog_Entry :=
                 Get_By_Id (H.Ctrl.DB, Result.Catalog_Id);
               Ok        : constant Boolean := Update_On_Success (H.Ctrl.DB, Cat_Entry.Id, Cat_Entry.Target_Version);
            ...
            declare
               use Podmander.Controller.Service_Catalog.Repository;
               Ok : constant Boolean := Update_On_Failure (H.Ctrl.DB, Result.Catalog_Id);
               pragma Unreferenced (Ok);
            begin
               Podmander.Logging.Warning
                 ("controller",
         ...
         if Result.Code = Podmander.Messages.Result_Codes.Ok then
            ...
            Podmander.Logging.Warning
              ("controller",
               "Deploy failed for " & To_String (Result.Service_Name) & ": " & To_String (Result.Error_Message));
         end if;
      end if;
   end Handle_Deploy_Result;

   overriding
   procedure Handle_Status_Query (H : in out Controller_Handler; M : Podmander.Messages.Status_Query_Type'Class) is
      pragma Unreferenced (H, M);
   begin
      Podmander.Logging.Warning ("controller", "Status_Query is controller-to-agent only");
   ...
   begin
      if Resp.Code = Podmander.Messages.Result_Codes.Ok then
         Podmander.Logging.Info ("controller", "Agent status: " & To_String (Resp.Containers));
      else
         Podmander.Logging.Warning
           ("controller",
            "Agent status query "
