...
package body Podmander.Controller.Service.Repository is
   ...
   function Row_To_Service_Version
     (DB : in out DB_Handle; QH : in out Query_Handle) return Podmander.Controller.Service_Version
   is
   ...
   begin
      ...
      Parse_Volume_Array (Column_Text (QH, 6), Result.Volumes, Result.Volumes_Count);
   ...
   exception
      when Constraint_Error =>
         raise Database_Error
           with
             Format_Error
               ((Kind => Unknown, Message => To_Unbounded_String ("Invalid version number in database"), Code => 0));
   ...
   function Create (DB : in out DB_Handle; Name : String) return Service is
      QH  : Query_Handle := Prepare (DB, "INSERT OR IGNORE INTO services (name) VALUES (?)");
      SEL : Query_Handle := Prepare (DB, "SELECT id, name FROM services WHERE name = ?");
   begin
      Bind_Text (QH, 1, Name);
      while Step (QH) loop
         null;
      ...
      if Step (SEL) then
         return (Id => Column_Int (SEL, 0), Name => To_Unbounded_String (Column_Text (SEL, 1)));
      else
         raise Database_Error
           with
             Format_Error
               ((Kind    => Unknown,
                 Message => To_Unbounded_String ("Failed to retrieve service after insert: " & Name),
                 Code    => 0));
      end if;
   end Create;
   ...
   function Get_By_Name (DB : in out DB_Handle; Name : String) return Service is
      QH : Query_Handle := Prepare (DB, "SELECT id, name FROM services WHERE name = ?");
   begin
      Bind_Text (QH, 1, Name);
      if Step (QH) then
         return (Id => Column_Int (QH, 0), Name => To_Unbounded_String (Column_Text (QH, 1)));
      else
         raise Database_Error
           with
             Format_Error
               ((Kind => Not_Found, Message => To_Unbounded_String ("Service not found: " & Name), Code => 0));
      end if;
   end Get_By_Name;
   ...
   function Get_By_Id (DB : in out DB_Handle; Id : Integer) return Service is
      QH : Query_Handle := Prepare (DB, "SELECT id, name FROM services WHERE id = ?");
   begin
           ...
           with
             Format_Error
               ((Kind    => Not_Found,
                 Message =>
                   To_Unbounded_String
                     ("Service not found by id: " & Ada.Strings.Fixed.Trim (Id'Image, Ada.Strings.Left)),
                 Code    => 0));
      end if;
   end Get_By_Id;
      ...
      QH : Query_Handle :=
        Prepare
          (DB,
           ...
           & "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
   begin
      Bind_Text (QH, 1, Ada.Strings.Fixed.Trim (Version.Service_Id'Image, Ada.Strings.Left));
      Bind_Text (QH, 2, Ada.Strings.Fixed.Trim (Version.Version'Image, Ada.Strings.Left));
      Bind_Text (QH, 3, To_String (Version.Image));
      Bind_Text (QH, 4, Env_Array_To_JSON (Version.Env, Version.Env_Count));
      Bind_Text (QH, 5, Port_Array_To_JSON (Version.Ports, Version.Ports_Count));
      Bind_Text (QH, 6, Volume_Array_To_JSON (Version.Volumes, Version.Volumes_Count));
   ...
   begin
      ...
      Bind_Text (QH, 2, Ada.Strings.Fixed.Trim (Version'Image, Ada.Strings.Left));
      if Step (QH) then
