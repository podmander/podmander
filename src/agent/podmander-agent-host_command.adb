...
package body Podmander.Agent.Host_Command is
   ...
   package body Internals is
      ...
      procedure Finalize (Self : in out Argument_Owner) is
         procedure Free_Arg is new
           Ada.Unchecked_Deallocation (Spoon.Argument, Spoon.Argument_Access);
      begin
   ...
   is
      Stdout : constant SU.Unbounded_String :=
        Spoon.Output.Text_Capturer (Text).Get (Spoon.Standard_Output);
      Stderr : constant SU.Unbounded_String :=
        Spoon.Output.Text_Capturer (Text).Get (Spoon.Standard_Error);
   begin
      case Raw.State is
         when Spoon.Exited     =>
                 ...
                 (if Err_To_Out
                  then
                    SU.To_Unbounded_String
                      (SU.To_String (Stdout) & SU.To_String (Stderr))
                  else Stdout),
               Error_Output =>
                 (if Err_To_Out then SU.Null_Unbounded_String else Stderr));

         when Spoon.Error      =>
            Podmander.Logging.Error
              ("host-command",
               "Spawn error"
               & Integer'Image (Raw.Error_Code)
               & " for "
               & Program);
            return
